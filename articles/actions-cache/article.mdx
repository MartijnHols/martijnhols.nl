...
I was an early user of Github Actions, and an <Annotation annotation="It might have helped that working with TravisCI was frustrating, and at the time its future was unclear as they had just been acquired by a party which seemed to want to go in a different direction.">immediate fan</Annotation>. I implemented it in all my projects, both private and [public](https://github.com/WoWAnalyzer/WoWAnalyzer/blob/e436c5c78f6461e4bc12e4649e88a8c99d189c6e/.github/workflows/build.yml#L20), which alone lead to over 200 developers relying on pipelines I set up in one way, shape or form.
...
- It's config syntax is the <Annotation annotation="I'm still waiting for a CI that uses TypeScript for configuration, with type safety, being able to use references instead of copy-pasting, and loops. Declarative config sounds great in theory for its simplicity, but when it keeps growing (and you keep running into copy-paste issues), it quickly starts to suck.">least bad</Annotation> of all the CI/CD systems I've used
